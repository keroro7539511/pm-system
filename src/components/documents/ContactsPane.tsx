...
function ContactCard({
  ...
  const fields: { icon: React.FC<{ className?: string }>; label: string; value: string | null }[] = [
    { icon: Mail,       label: t("contacts.fields.email"),   value: contact.email },
    { icon: Phone,      label: t("contacts.fields.phone"),   value: contact.phone },
    { icon: Building2,  label: t("contacts.fields.company"), value: contact.company_name },
    { icon: MapPin,     label: t("contacts.fields.address"), value: contact.company_address },
    { icon: FolderOpen, label: t("contacts.fields.project"), value: contact.project_name },
  ];
    ...
    <div className="flex flex-col h-full overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          {deleteConfirm ? (
            <div className="flex items-center gap-1">
              <Button size="sm" variant="destructive" onClick={onDelete} disabled={isDeleting} className="text-xs h-7">{t("common.deleteConfirmBtn")}</Button>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-lg space-y-4">
          ...
          <p className="text-xs text-text-muted mt-6">{t("common.createdAt", { date: formatDate(contact.created_at) })}</p>
        </div>
      </div>
    </div>
  );
...
export function ContactsPane() {
  ...
  function handleDelete() {
    ...
    deleteContact.mutate(selected.id, {
      onSuccess: () => { setSelected(null); setDeleteConfirm(false); toast(t("contacts.deleted"), "success"); },
    });
  }
      ...
      <div className="w-72 shrink-0 border-r border-border flex flex-col bg-sidebar">
        <div className="p-4 border-b border-border space-y-3">
          <div className="flex items-center justify-between gap-2">
            <h1 className="text-lg font-semibold text-text-primary shrink-0">{t("contacts.title")}</h1>
            <div className="flex items-center gap-1">
          ...
          <div className="relative">
            ...
            <Input placeholder={t("contacts.searchPlaceholder")} value={search}
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {isLoading ? (
            ...
            <div className="p-4 text-center">
              ...
              <p className="text-sm text-text-muted">{t("contacts.noContacts")}</p>
              <p className="text-xs text-text-muted mt-1">{t("contacts.noContactsDesc")}</p>
            </div>
          ) : (
      ...
      <div className="flex-1 overflow-hidden">
        {selected ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-text-muted">
            ...
            <p className="text-lg">{t("contacts.selectPrompt")}</p>
            <p className="text-sm mt-1">{t("contacts.orAdd")}</p>
          </div>
        )}
      </div>
